<template>
  <div class="flex flex-col md:px-32 lg:px-44 items-start mb-10 md:mb-24 mx-4" id="experience">
      <div class="flex md:py-2 content-center my-4">
        <p class="font-bold font-sans text-lg md:text-xl text-gray-500">
          <span class="text-gray-400 ml-2 text-lg md:text-xl font-mono"> 02. </span> Where I've Worked
        </p>
        <hr class="w-/4 font-semibold mx-4 md:mx-6 mt-4 bg-gray-400 " />
      </div>
    <div class="flex flex-col md:flex-row overflow-x-scroll md:overflow-visible">
...
import tabGoogle from "./Where/tabGoogle.vue";
import tabFacebook from "./Where/tabFacebook.vue";
...
export default {
  ...
  setup() {
    ...
    const currentPlace = ref("Apple");
    const currentPlaceComponent = computed(
      () => "tab-" + currentPlace.value.toLowerCase()
    );
